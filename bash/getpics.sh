...
## check to see if file already exists, retrieve it if it does not
test -f ~/public_html/pics/pics.tgz || wget -q -O ~/public_html/pics/pics.tgz http://zonzorp.net/pics.tgz || (echo "File unavailable." && exit 1)
## decompress file
tar -xf ~/public_html/pics/pics.tgz -C ~/public_html/ 2>/dev/null && rm -r ~/public_html/pics/pics.tgz || (echo "Failed to extract files." && exit 1)
