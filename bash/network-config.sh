...
#   To separate it and make it easier to read, we should take those commands and put them before the output generation, in their own
#   section that generates the data and saves it in variables
...
###
##### Get router IP by checking default route on host
##### Get router Name through hosts database
###
router=$(ip route | grep "default" | cut -d" " -f3)
rHost=$(getent hosts $router | awk '{print $2}')

###
##### Get interface name & IP through IP command, after removing unwanted output
####
iface=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
ip=$(ip a s $iface | awk '/inet /{gsub(/\/.*/,"");print $2}')

###
##### Get hostname through hosts database
###
hostName=$(getent hosts $ip | awk '{print $2}')

###
##### Get external IP and Name through external website & hosts database
###
