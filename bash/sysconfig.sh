...
function displayhelp {
	echo -e "Usage: sysconfig [ --help|-h ] [--host] [--domain] [--os] [--ipconfig] [--cpu] [--memory] [--disk] [--printer] \
		\nWHERE \
		\n--help|-h:  display help \
		\n--host:  display only host information \
		\n--domain:  display only domain information \
		\n--os:  display only operating system information \
		\n--ipconfig:  display only IP information \
		\n--cpu:  display only CPU information \
		\n--memory:  display only memory information \
		\n--disk:  display only disk information \
		\n--printer:  display only printer information"
}
