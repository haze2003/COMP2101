...
###
##### Get interfaces on host
###
iface=$(ip a | cut -d" " -f2 | awk '/.*:/' | sed 's/://')


###
##### Ensure no more than 2 arguments are supplied, otherwise exit
###
[ $# -gt 2 ] && echo -e "Usage: netid [ -v ] INTERFACENAME\nwhere -v:  be verbose\nINTERFACENAME:  name of host interface" && exit 1

###
##### Check command line arguments for validity and actions needed
###
for arg in $@; do
	case "$arg" in
		-v)
		...
		;;
		*)
		for intFace in $iface; do
			if [ "$arg" = "$intFace" ]; then  #  Check to see if supplied interface exists on system
			onlyOneInterface=yes
			cliface=$intFace
			break
			fi
		done
		# Exit script if interface supplied is invalid
		[ "$onlyOneInterface" != "yes" ] &&  echo -e "Usage: netid [ -v ] INTERFACENAME\nwhere -v:  be verbose\nINTERFACENAME:  name of host interface" && exit 1 
		;;
	esac
done
...
###
##### Start loop to display all interfaces on host,
##### UNLESS 1 host was specified on the command line
###

for ifaceName in $iface; do

	[ "$ifaceName" = "lo" ] && continue  #skip loopback device

	interface=$ifaceName

	if [ "$onlyOneInterface" = "yes" ]; then  #check if only oen interface should be displayed
		interface=$cliface
		runOnce=yes
	fi
...
#   e.g. grep -q mynetworknumber /etc/networks || (echo 'mynetworkname mynetworknumber' |sudo tee -a /etc/networks)
	...
	Host Name	: $ipv4_hostname
	Network Address : $network_address
	Network Name    : $network_name

EOF
	[ "$runOnce" = "yes" ] && break  #stop from looping to other interfaces if one interface was specified
done
